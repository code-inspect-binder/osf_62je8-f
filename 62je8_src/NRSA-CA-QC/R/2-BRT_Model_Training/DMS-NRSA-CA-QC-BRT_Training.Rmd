...
#load("R/2-BRT_Model_Training/DMS-NRSA-CA-QC-BRT_Training_Data.RData")
...
Tuning values for each BRT:

* Number of trees = 1000, 5000, 7500, 10000, 20000
* Interaction depth = 2, 3
* Minimum observation per node = 5, 10, 15, 20, 25, 50
* Shrinkage = 0.0001, 0.0005, 0.001
  ...
  function(j){
  	...
  	require(gbm)
 	  ...
 	  BRT.training <- train(site.scores ~ NO3 + NH4 + total.P + pH.lab + cond + DOC
			  								  ...
			  								  method = "gbm", 
				  							  verbose = FALSE,
					  						  tuneGrid = expand.grid(n.trees = c(1000, 5000, 7500, 10000, 20000), 
						  									     interaction.depth = c(2:3), 
							  								     shrinkage = c(0.0001, 0.0005, 0.001),
								  							     n.minobsinnode = c(5, 10, 15, 20, 25, 50)
						  									     )
									  		)
...
kable(CA.BRT.training.output, 
			booktabs = TRUE,
			digits = 4,
			caption = "Model training results for the BRTs.") %>% 
	kable_styling(latex_options = c("HOLD_position", "striped"))
```



```{r Save BRT Model Training Output, eval = FALSE, include = FALSE}
## Save workspace
save.image("R/2-BRT_Model_Training/DMS-NRSA-CA-QC-BRT_Training_Output.RData")
```


```{r Computational Time, include = FALSE}
## Computational time (3 cores)
# Start =  8:08pm, 18 March 2021
# End   = 12:25am, 19 March 2021
```
